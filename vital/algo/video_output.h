...
public:
  ...
  /// \param frame_rate Global frame rate for output video.
  ///
  /// \throws exception Thrown if opening the video stream failed.
  virtual void open( std::string video_name, double frame_rate = 0.0 ) = 0;
  ...
  virtual void add_image(
    ...
    kwiver::vital::image_container_sptr const& image ) = 0;
